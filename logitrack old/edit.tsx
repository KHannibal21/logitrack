// app/(courier)/edit.tsx
// ✅ IDEAL SENIOR Courier Edit Profile (no libs)
...
export default function CourierEditProfileScreen() {
  ...
  const initialPhoneDigits = useMemo(() => {
    // baseline: берём только цифры, и если было 8XXXXXXXXXX (11) — приводим к 7..., чтобы dirty сравнение было честнее
  ...
  // держим refs синхронно со state
  useEffect(() => {
  ...
  // soft error показываем только если уже 11+ цифр (чтобы не бесить во время ввода)
  ...
  const handleBack = useCallback(() => {
    if (!isDirty || saving) {
      resetForm();
      if (router.canGoBack()) router.replace('/(courier)/details');
  ...
  // ✅ Сеньорский обработчик маски:
  // 1) вытаскиваем digits из нового текста
  // 2) если digits не изменились, но строка стала короче => удалили символ маски => удаляем последнюю цифру
  const handlePhoneChange = useCallback((text: string) => {
    ...
    // пользователь "стирает скобку/пробел/дефис", digits не меняются => надо удалить цифру, иначе маска вернётся
  ...
  const handleSave = useCallback(async () => {
    ...
    if (normalized) {
      payload.phone = formatPhoneDisplay(normalized);
    } else {
      payload.phone = '';
    }
  ...
  if (!uid) {
    return (
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        >
          <TouchableOpacity onPress={() => router.replace('/(courier)')} style={styles.headerBtn} activeOpacity={0.85}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        ...
        >
          <View style={[styles.card, { backgroundColor: colors.card }]}>
            <ThemedText style={[styles.sectionTitle, { color: colors.text }]}>Данные курьера</ThemedText>
