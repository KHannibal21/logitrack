...
export default function CourierProfileScreen() {
  ...
  const { isTracking, error: locationError } = useLocation();
  ...
  const [editingBankCard, setEditingBankCard] = useState('');
  ...
  const handleEditPress = useCallback(() => {
    ...
    setEditingVehicle(user?.vehicle || '');
    setEditingBankCard((user as any)?.preferredBankCard || '');
    setEditModalVisible(true);
  }, [user?.name, user?.phone, user?.vehicle]);
  ...
  const handleSaveEdit = useCallback(async () => {
    ...
    try {
      if (user?.uid) {
        const updates: Record<string, any> = { name: editingName.trim() };
        if (editingPhone) updates.phone = editingPhone;
        if (editingVehicle) updates.vehicle = editingVehicle;
        if (editingBankCard) updates.preferredBankCard = editingBankCard;
  ...
  }, [user?.uid, editingName, editingPhone, editingVehicle, editingBankCard]);
        ...
        { text: t('common.cancel'), style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              Alert.alert(t('error'), t('profile.logoutError'));
            } finally {
              setLoading(false);
            }
          },
        ...
        <View style={styles.profileHeaderContent}>
          <View style={styles.profileHeaderLeft}>
            ...
            <View>
              ...
              <ThemedText style={styles.profileHeaderRole}>Курьер</ThemedText>
            </View>
            </View>
          <TouchableOpacity
            ...
            <View style={styles.statusRow}>
              ...
              <View style={styles.statusTextContainer}>
                <ThemedText style={styles.statusTitle}>
                  {isTracking ? t('profile.geolocationActive') : t('profile.geolocationInactive')}
                </ThemedText>
                {locationError && (
                  <ThemedText style={[styles.statusError, { color: colors.error }]}>
                    {locationError}
                  </ThemedText>
                )}
              </View>
          ...
          <Card variant="elevated" style={styles.infoCard}>
            <View style={styles.infoRow}>
              ...
              <View style={styles.infoContent}>
                <ThemedText style={styles.infoLabel}>{t('users.email')}</ThemedText>
            ...
            {user.phone && (
              <View style={styles.infoRow}>
                ...
                <View style={styles.infoContent}>
                  <ThemedText style={styles.infoLabel}>{t('profile.phone')}</ThemedText>
            ...
            {user.vehicle && (
              <View style={styles.infoRow}>
                <View style={styles.infoIcon}>
                  <Ionicons name="car-outline" size={20} color={colors.icon} />
                </View>
                <View style={styles.infoContent}>
                  <ThemedText style={styles.infoLabel}>{t('profile.vehicleLabel')}</ThemedText>
                  <ThemedText style={styles.infoValue}>{user.vehicle}</ThemedText>
                </View>
              </View>
            )}

            {(user as any)?.preferredBankCard && (
              <View style={styles.infoRow}>
                <View style={styles.infoIcon}>
                  <Ionicons name="card-outline" size={20} color={colors.icon} />
                </View>
                <View style={styles.infoContent}>
                  <ThemedText style={styles.infoLabel}>{t('profile.bankCardLabel') || 'Карта для переводов'}</ThemedText>
                  <ThemedText style={styles.infoValue}>{(user as any).preferredBankCard}</ThemedText>
                </View>
              </View>
            )}
          </Card>
        ...
        >
          ...
          >
            <View style={styles.modalHeader}>
              <ThemedText type="subtitle">{t('profile.editTitle')}</ThemedText>
            ...
            >
              ...
              />
              <Input
                label={t('profile.vehicleLabel')}
                placeholder={t('profile.vehiclePlaceholder')}
                value={editingVehicle}
                onChangeText={setEditingVehicle}
              />
              <Input
                label={t('profile.bankCardLabel') || 'Карта/Счёт для переводов'}
                placeholder={t('profile.bankCardPlaceholder') || 'например, Kaspi Gold ****1234'}
                value={editingBankCard}
                onChangeText={setEditingBankCard}
              />
            </ScrollView>
